
<?php
use Mpdf\Tag\Center;
    // $nomor = $_POST['nomor'];
    // $nama = $_POST['nama'];
    // $ttl = $_POST['ttl'];
    // $kebangsaan = $_POST['kebangsaan'];
    // $kelamin = $_POST['kelamin'];
    // $status = $_POST['status'];
    // $pekerjaan = $_POST['pekerjaan'];
    // $nik = $_POST['nik'];
    // $alamat = $_POST['alamat'];
    // $tanggalsurat = $_POST['tanggalsurat'];

    require("../Web_Kelurahan_Kepuharjo/fpdf185/fpdf.php");
 
    $pdf = new FPDF();
    $pdf->AddPage();
    $pdf->Image('../Web_Kelurahan_Kepuharjo/images/logohp.png',18,8,50);
    $pdf->SetFont('Times','',14);
    $pdf->SetXY(70,18);
    $pdf->SetFontSize(14);
    $pdf->Write(0,'PEMERINTAH KABUPATEN LUMAJANG');
    $pdf->SetXY(85,24);
    $pdf->SetFontSize(14);
    $pdf->Write(0,'KECAMATAN LUMAJANG');
    $pdf->SetFont('Times','',20);
    $pdf->SetXY(75,30);
    $pdf->SetFontSize(18);
    $pdf->Write(0,'KELURAHAN KEPUHARJO');
    $pdf->SetFont('TIMES','',14);
    $pdf->SetXY(75,36);
    $pdf->SetFontSize(14);
    $pdf->Write(0,'Jl. Langsep No. 18 Telp. (0334) 888243');
    $pdf->SetFont('TIMES','',14);
    $pdf->SetXY(99,42);
    $pdf->SetFontSize(14);
    $pdf->Write(0,'LUMAJANG');
    $pdf->SetXY(10,20);
    $pdf->SetFont('Times','B',14);
    $pdf->Cell(0,80,'SURAT KETERANGAN DOMISILI',0,0,'C');

    $pdf->SetFont('Times','');
    $pdf->SetXY(70,62);
    $pdf->SetFontSize(12);
    $pdf->Write(10,'Nomor : ');
    // $pdf->Write(10,$nomor);
    $pdf->SetFont('Times','');
    $pdf->SetXY(40,80);
    $pdf->SetFontSize(12);
    $pdf->Write(0,'Yang bertanda tangan di bawah ini kami Lurah Kepuharjo Kecamatan ');
    $pdf->SetFont('Times','');
    $pdf->SetXY(25,86);
    $pdf->SetFontSize(12);
    $pdf->Write(0,'Lumajang Kabupaten Lumajang menerangkan bahwa : ');
    $pdf->SetXY(25,96);
    $pdf->SetFontSize(12);
    $pdf->Write(0,'               Nama                                : ');
    // $pdf->Write(0,$nama);
    $pdf->SetXY(25,102);
    $pdf->SetFontSize(12);
    $pdf->Write(0,'               Tempat, Tanggal Lahir     : ');
    // $pdf->Write(0,$ttl);
    $pdf->SetXY(25,108);
    $pdf->SetFontSize(12);
    $pdf->Write(0,'               Kebangsaan/Agama          : ');
    // $pdf->Write(0,$kebangsaan);
    $pdf->SetXY(25,114);
    $pdf->SetFontSize(12);
    $pdf->Write(0,'               Jenis Kelamin                   : ');
    $pdf->SetXY(25,120);
    $pdf->SetFontSize(12);
    $pdf->Write(0,'               Status                                : ');
    // $pdf->Write(0,$status);
    $pdf->SetXY(25,126);
    $pdf->SetFontSize(12);
    $pdf->Write(0,'               Pekerjaan                          : ');
    // $pdf->Write(0,$pekerjaan);
    $pdf->SetXY(25,132);
    $pdf->SetFontSize(12);
    $pdf->Write(0,'               NIK                                   : ');
    // $pdf->Write(0,$nik);
    $pdf->SetXY(25,138);
    $pdf->SetFontSize(12);
    $pdf->Write(0,'               Alamat                              : ');
    // $pdf->Write(0,$alamat);
    $pdf->SetXY(40,148);
    $pdf->SetFontSize(12);
    $pdf->Write(0,'Adalah benar sampai dengan saat ini warga kami dan berdasarkan surat pengantar');
    $pdf->SetXY(25,154);
    $pdf->SetFontSize(12);
    $pdf->Write(0,'_________ tanggal __________ Nomor : _____________ dan pengkuannya. Menerangkan');
    $pdf->SetXY(25,160);
    $pdf->SetFontSize(12);
    $pdf->Write(0,'bahwa nama tersebut diatas benar berdomisili di __________. Surat Keterangan Domisili');
    $pdf->SetXY(25,166);
    $pdf->SetFontSize(12);
    $pdf->Write(0,'ini dipergunakan untuk ___________');
    $pdf->SetXY(40,172);
    $pdf->SetFontSize(12);
    $pdf->Write(0,'Demikian surat keterangan ini kami buat untuk dapat dipergunakan sebagaimana ');
    $pdf->SetXY(25,178);
    $pdf->SetFontSize(12);
    $pdf->Write(0,'mestinnya.');
    $pdf->SetXY(45,198);
    $pdf->SetFontSize(12);
    $pdf->Write(0,'                                                              Lumajang, ');
    // $pdf->Write(0,$tanggalsurat);
    $pdf->SetXY(35,204);
    $pdf->SetFontSize(14);
    $pdf->Write(0,'                                                             LURAH KEPUHARJO');
    $pdf->SetXY(35,210);
    $pdf->SetFontSize(14);
    $pdf->Image('../Web_Kelurahan_Kepuharjo/images/ttd.png',120,215,50);
    $pdf->Image('../Web_Kelurahan_Kepuharjo/images/stempel.png',110,208,40);
    $pdf->SetXY(35,250);
    $pdf->SetFontSize(14);
    $pdf->Write(0,'                                                            MUHAMMAD SAIFUL,S.AP');

    // $pdf->Write(100,$name);
    // $pdf->Write(100,$email);
    // $pdf->Write(100,$mobile);
 
    // $file = time().'.pdf';
    $pdf->output();
?>