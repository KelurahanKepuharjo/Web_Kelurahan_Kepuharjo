...
class domisili extends koneksii
{
    public function suratdiproses()
    {
        $sql = "SELECT id_surat, no_surat, nama, tempat_lahir, tanggal_lahir, jenis_kelamin, kebangsaan, agama, status_perkawinan, pekerjaan, nik, alamat, domisili.RT, domisili.RW, no_pengantar_surat, tgl_surat_pengantar, alamat_domisili_kel_kepu, surat_digunakan_untuk, tgl_surat_dibuat, status_surat, domisili.images, akun.no_hp, domisili.id_akun FROM domisili JOIN akun ON domisili.id_akun = akun.id_akun WHERE domisili.status_surat = 'Diproses'";
        $result = $this->koneksi->prepare($sql);
        $result->execute();
        return $result;
    }
...
class belum_menikah extends koneksii
{
    public function suratdiproses()
    {
        $sql = "SELECT `id_surat`, `no_surat`, `nama`, `tempat_lahir`, `tanggal_lahir`, `jenis_kelamin`, `kebangsaan`, `agama`, `status`, `pekerjaan`, `nik`, `alamat`, `status_surat`, `tgl_pengajuan`, surat_ket_belum_menikah.RT, surat_ket_belum_menikah.RW, `surat_digunakan`, surat_ket_belum_menikah.image, akun.id_akun, akun.no_hp FROM `surat_ket_belum_menikah` JOIN akun ON akun.id_akun = surat_ket_belum_menikah.id_akun WHERE status_surat='diproses'";
        $result = $this->koneksi->prepare($sql);
        $result->execute();
        return $result;
    }
...
class ket_usaha extends koneksii
{
    public function suratdiproses()
    {
        $sql = "SELECT `id_surat`, `no_surat`, `nama`, `tempat_lahir`, `tanggal_lahir`, `jenis_kelamin`, `kebangsaan`, `agama`, `status`, `pekerjaan`, `nik`, `alamat`, `nama_usaha`, `keperluan`, `tgl_surat_dibuat`, surat_usaha.image, `status_surat`, surat_usaha.RT, surat_usaha.RW, `tgl_pengajuan`, surat_usaha.id_akun, akun.id_akun, akun.no_hp FROM `surat_usaha` JOIN akun ON surat_usaha.id_akun=akun.id_akun WHERE status_surat='diproses'";
        $result = $this->koneksi->prepare($sql);
        $result->execute();
        return $result;
    }
...
class pindah extends koneksii
{
    public function suratdiproses()
    {
        $sql = "SELECT `id_surat`, `tgl_dibuat`, `no_surat`, `sifat`, `lampiran`, `perihal`, `nama`, `tempat_lahir`, `tanggal_lahir`, `jenis_kelamin`, `status`, `agama`, `alamat_asal`, `alamat_tujuan`, `kelurahan`, `kecamatan`, `kabupaten`, `provinsi`, `alasan_pindah`, `pengikut`, `status_surat`, `tgl_pengajuan`, surat_pindah.RT, surat_pindah.RW, surat_pindah.image, surat_pindah.id_akun FROM `surat_pindah` JOIN akun ON surat_pindah.id_akun=akun.id_akun WHERE status_surat='diproses'";
        $result = $this->koneksi->prepare($sql);
        $result->execute();
        return $result;
    }
